...
/*   Updated: 2021/02/21 15:04:59 by ybong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_split_str(char *str, int fd, char **read_sofar, char **leftover, char **line)
{
	char		*temp;
...
int		get_next_line(int fd, char **line)
{
	...
	*read_sofar = ft_strdup("");
	if (leftover[fd])
	{
		if (0 != (res = ft_split_str(leftover[fd], fd, read_sofar, leftover, line)))
			return (res);
	}//여기까지 이전 read의 leftover에 있는 값을 처리하는 과정
	while ((readbyte = read(fd, buf, BUFFER_SIZE)) > 0) // EOF를 만나기 전의 경우 (EOF 읽으면 while문 종료)
	{
		buf[readbyte] = 0;
		if (0 != (res = ft_split_str(buf, fd, read_sofar, leftover, line)))
			return (res);
